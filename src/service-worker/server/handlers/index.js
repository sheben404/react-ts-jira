...
const sleep = (t = ls("__min_request_time__", 200)) =>
...
export const handlers = [
  ...
  return {
    ...
    async resolver(req, res, ctx) {
      try {
        if (shouldFail(req)) {
          throw new Error("请求失败，请检查 dev-tool 的设置");
        }
...
function shouldFail(req) {
  ...
  const failureRate = Number(
    window.localStorage.getItem("__failure_rate__") || 0
  );
  if (Math.random() * 100 < failureRate) return true;
...
function requestMatchesFailConfig(req) {
  ...
  try {
    const failConfig = JSON.parse(
      window.localStorage.getItem("__request_fail_config__") || "[]"
    );
    if (failConfig.some(configMatches)) return true;
  } catch (error) {
    window.localStorage.removeItem("__request_fail_config__");
  }
  return false;
}
